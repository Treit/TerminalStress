...
/// <summary>
/// CLI tool for managing the StressBotDaemon Windows service.
/// Invoked when the exe is run with a subcommand (install, uninstall, start, stop, status, config).
/// </summary>
public static class CliTool
{
    private const string ServiceName = "StressBotDaemon";
    private const string DisplayName = "StressBot Agent Daemon";
    private const string Description = "Auto-starts the TerminalStress agent daemon on boot.";
    ...
    private static int PrintUsage(string? error = null)
    {
        ...
        Console.WriteLine("""
            StressBotService — Windows Service for the agent daemon

            Usage:
              StressBotService <command> [options]
            ...
            Config keys:
              ...
              maxRestartWindowMinutes, logDirectory, enabled

            Examples:
              StressBotService install
              StressBotService config set arguments "--interval 15"
              StressBotService config set restartDelaySeconds 10
              StressBotService status
            """);
    ...
    private static int Install()
    {
        ...
        Console.WriteLine();
        Console.WriteLine("  To start now:  StressBotService start");
        Console.WriteLine("  To configure:  StressBotService config show");
    ...
    private static int ShowStatus()
    {
        ...
        if (File.Exists(configPath))
        {
            ...
            Console.WriteLine($"  Working dir:     {cfg.WorkingDirectory}");
            ...
            if (!File.Exists(cfg.ResolvedScriptPath))
        ...
        try
        {
            ...
            // Rough check — we can't easily read command line of other processes
    ...
    private static int ConfigSet(string key, string value)
    {
        ...
        try
        {
            ...
            Console.WriteLine("    StressBotService stop && StressBotService start");
        }
        catch (Exception ex)
        {
