...
public class DaemonWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation(
            "StressBot service starting. Python={Python}, Script={Script}, WorkDir={Dir}",
            _config.ResolvedPythonPath, _config.ResolvedScriptPath, _config.WorkingDirectory);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            _logger.LogWarning(
                "Daemon exited. Restarting in {Delay}s...", _config.RestartDelaySeconds);

            await Task.Delay(TimeSpan.FromSeconds(_config.RestartDelaySeconds), stoppingToken);
        }
    }
    ...
    private void ValidateConfig()
    {
        ...
        if (!File.Exists(_config.ResolvedScriptPath))
            _logger.LogWarning("Script not found at {Path}", _config.ResolvedScriptPath);

        if (!Directory.Exists(_config.WorkingDirectory))
            _logger.LogWarning("Working directory not found: {Path}", _config.WorkingDirectory);
    }

    /// <summary>
    /// Load key=value pairs from .env in the working directory into the
    /// process start info environment, so the daemon gets STORAGE_CONNECTION_STRING, etc.
    /// </summary>
    private void LoadDotEnv(ProcessStartInfo psi)
    {
        var envFile = Path.Combine(_config.WorkingDirectory, ".env");
    ...
    /// can find tools like copilot that live in the user profile.
    /// </summary>
    ...
    /// <summary>
    /// Set USERPROFILE, HOMEPATH, APPDATA, LOCALAPPDATA so that tools like copilot
    /// can find auth tokens and config when running under the SYSTEM account.
    ...
    {
        _logger.LogInformation("StressBot service stopping...");
        KillDaemon();
